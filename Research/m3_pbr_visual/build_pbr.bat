...
cl /nologo /std:c++17 /O2 /EHsc /MD /DENABLE_D3D12 /DNOMINMAX /DGLM_ENABLE_EXPERIMENTAL ^
  /I"%PBR%\external\AgilitySDK717\build\native\include" ^
  /I"%PBR%\external\DXC2505\build\native\include" ^
  ...
  /LIBPATH:"%PBR%\external\DXC2505\build\native\lib\x64" ^
  glfw3dll.lib assimp.lib d3d12.lib dxgi.lib d3dcompiler.lib dxcompiler.lib ^
...
copy /y "%PBR%\external\DXC2505\build\native\bin\x64\dxcompiler.dll" . >nul
copy /y "%PBR%\external\DXC2505\build\native\bin\x64\dxil.dll" . >nul
if not exist D3D12 mkdir D3D12
copy /y "%PBR%\external\AgilitySDK717\build\native\bin\x64\D3D12Core.dll" D3D12\ >nul
